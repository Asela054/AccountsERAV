					...
					<div class="page-header-content py-3">
						<h1 class="page-header-title font-weight-light">
                            ...
                            <div class="dropdown-menu" aria-labelledby="btnorderacts">
                            	...
                            	<a class="dropdown-item" id="btnOrderCreate" href="javascript:void(0);" title="Create new payment" data-mainid="-1">New</a>
                        ...
                        <div class="card-body p-0 p-2">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="text" id="txt_supp_invoice_amount" class="form-control form-control-sm running_tot_figures" value="0" />
                                </div>
                                <div class="col-md-2">
                            ...
                            <div class="row">
                                <div class="col-md-4">
									...
									$childAccountAttr=array('id'=>'drp_child_account_list', 
										 ...
										 'required'=>'required');
									echo form_dropdown('drp_child_account_list', $child_accounts, '-1', $childAccountAttr); 
									*/
									?>
                                    <select class="form-control form-control-sm running_tot_figures" id="drp_child_account_list" name="drp_child_account_list" required="required" >
                                    	<option value="-1" data-parentacc="">Select</option>
                                        <?php 
										foreach($child_accounts as $opt_ch){
										?>
                                        <option value="<?php echo $opt_ch->form_key; ?>" data-parentacc=""><?php echo $opt_ch->form_val; ?></option>
                                        <?php 	
										}
										?>
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="text" id="txt_acc_detail_amount" class="form-control form-control-sm running_tot_figures" value="" />
                                </div>
                            </div>
...
$(document).ready(function(){
	var providersTable = $('#providers_dataTable').DataTable({ 
				  ...
				  "columns":[
						...
						{"data":"parent_company_regno", "className":'text-center', "width":'5%'}
					],
	...
	var allPaymentsTable = $('#allPayments_dataTable').DataTable({ 
				  ...
				  "columns":[
						{"data":"supplier_name"}, {"data":"pay_remarks"}, {"data":"pay_invoice", "width":'15%'}, 
						{"data":"pay_invoice_total", "className":'text-right', "width":'15%'}, //{"data":"parent_company_regno"}, 
				  ...
				  "columnDefs": [{
						...
						render: function( data, type, row ){
							...
							*/
							var invoice_value = parseFloat(data);
							return invoice_value.toFixed(2);//data;
	...
	*/
	$("#assignmentListModal").on("shown.bs.modal", function(){
		var c = $("#drp_company_list").find(":selected").val();//$("#drp_filter_company_payment_list").find(":selected").val();
		var d = $("#drp_company_branch_list").find(":selected").val();//$("#drp_filter_branch_payment_list").find(":selected").val();
		if (allPaymentsTable.column(4).search() !== (c+"_"+d)) {
			allPaymentsTable.column(4).search((c+"_"+d)).draw();
		}
	});
	...
	var accDetailTable = $("#accDetail_dataTable").DataTable({
				...
				"columns":[
						...
						{"data":"acc_payable_narration"}, {"data":"acc_payable_amount", "className":'text-right'}
					],
				"columnDefs":[{
							"targets":0,
							"render":function( data, type, row ){
								...
								return '<input type="checkbox" class="chk_accd" value="'+data+'" '+
									'data-payableid="'+row.acc_payable_id+'" data-revisestatus="0" '+
	...
	function calc_running_tot_figures(){
		...
		var fig_supp_invoice_amount = parseFloat($("#txt_supp_invoice_amount").val());
		
		var fig_acc_detail_amount = parseFloat($("#txt_acc_detail_amount").val());
		var child_account_id = $("#drp_child_account_list").find(":selected").val();
		
		var acc_detail_running_total = parseFloat($("#h_acc_detail_running_total").val());
		
		//console.log('#chk_accd-'+child_account_id);
		
		var c = accDetailTable.cell('tr#chk_accd-'+child_account_id+' '+'td:nth-child(1)').node();
		//console.log($(c).children('input[type="checkbox"]').is(":checked")?'y':'n');//($(c).html());
		var c_child = $(c).children('input[type="checkbox"]');
		
		var selected_tr = accDetailTable.row('#chk_accd-'+child_account_id);
		...
		if(selected_tr.length==1){
			...
			child_account_val = parseFloat(d.acc_payable_amount)*c_child_status;
		}
		...
		if(isNaN(fig_acc_detail_amount) || (child_account_id=='-1')){
	...
	function update_acc_detail_running_total(child_account_id){
		var acc_detail_running_total = parseFloat($("#h_acc_detail_running_total").val());
		if(isNaN(acc_detail_running_total)){
			acc_detail_running_total = 0;
		}
		...
		$("#h_acc_detail_running_total").val(acc_detail_running_total);
	}
	...
	$(document).on("click", "#btnOrderCreate, .btn_new_payment_x0409, .btn_edit_payment", function(){
		var mainid = $(this).data('mainid');
		var companyreg = $("#drp_company_list").find(":selected").val();//$(this).data('companyreg');
		var branchreg = $("#drp_company_branch_list").find(":selected").val();//$(this).data('branchreg');
		...
		$.ajax({
			...
			$("#h_acc_detail_running_total").val(data.payment_figures_total);//0
			$("#h_acc_payable_main_id").val(data.view_header_data.idtbl_account_payable_main);
			
			var short_val = parseFloat($("#txt_supp_invoice_amount").val())-parseFloat($("#h_acc_detail_running_total").val());
	...
	$("#frmPaymentInfo").submit(function(event){
		...
		var revisedRows = $('.chk_accd[data-revisestatus="1"]').map(function(i, el) {
							...
							var d = selected_tr.data();
							//console.log(d.acc_payable_amount);
							var selectedAccount = $("#drp_child_account_list").find(":selected").val();
							if(d.acc_detail_id != selectedAccount){
								var newstatus = $(this).is(":checked")?1:2;//1:0
								return {
									idtbl_account_payable:$(el).attr('data-payableid'), 
									status:newstatus,
		...
		$.ajax({
			...
			data:{company_id:companyId, company_branch_id:companyBranchId, supplier_id:supplierId,
				...
				acc_detail_running_total:accDetailRunningTotal, payment_shortage:paymentShortage, 
			...
			if(rres!=""){
				...
				$('.chk_accd[data-revisestatus="1"]').each(function(index, value){
					...
					var selected_tr = accDetailTable.row($(this).parent().closest('tr'));
					var d = selected_tr.data();
					d.acc_payable_status = newstatus;
				...
				update_acc_detail_running_total(childAccountId);
	...
	*/
	$("#btnOrderApprovePost").on("click", function(){
		if($("#h_acc_payable_main_id").val()!=''){
			var opt_approve = confirm('Are you sure you want to post this invoice?');
			...
			if(opt_approve){
				...
				$.ajax({
					method: "POST",
					url: "<?php echo base_url('AccountPayment/approve'); ?>",
					data:{revised_rows:revisedRows, selected_opt:mainid, exp_rows:0, recordID:mainid,
						supp_invoice_amount:suppInvoiceAmount},
					dataType:"JSON",
					beforeSend:function(){
						$("#lbl_warnmsg").html('<i class="fa fa-spinner fa-spin"></i>Please wait.');
					}
				}).done(function(data){
					resData = JSON.parse(data.action);
					$("#lbl_warnmsg").html(resData.message);//resMsg
				});
				
			}
		}else{
			alert("You need to create or open existing invoice first");
		}
	});
	...
	$(document).on('click', '.chk_accd', function(){
		...
		var d = selected_tr.data();
		//console.log(d.acc_payable_amount);
		var old_payable_status = d.acc_payable_status;
		...
		$.ajax({
			...
			if(data.toggle_status=='success'){
				...
				acc_detail_running_total += (parseFloat(d.acc_payable_amount)*(CHK_PAYABLE_STATUS+(CHK_PAYABLE_STATUS-1)));
				$("#h_acc_detail_running_total").val(acc_detail_running_total);
