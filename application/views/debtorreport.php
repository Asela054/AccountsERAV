                                    ...
                                    <div class="form-row">
                                        <div class="col-2 mb-1">
                                            ...
                                            <select class="form-control form-control-sm" name="reporttype" id="reporttype" required>
                                                ...
                                                <option value="1">Debtor Statement</option>
                                                <option value="2">Debtor Age Analysis</option>
                                            </select>
                                        </div>
...
<script>
    $(document).ready(function() {
        ...
        $("#customer").select2({
            ajax: {
                url: "<?php echo base_url() ?>Debtorreport/Getcustomerlist",
                type: "post",
                dataType: 'json',
                delay: 250,
                ...
                Swal.fire({
                    ...
                    didOpen: () => {
                        ...
                        $.ajax({
                            ...
                            url: 'Debtorreport/Debtorreportview',
        ...
        $('#btnpdfconvert').click(function(){
            ...
            var table = document.getElementById("debtorStatementTable");
                ...
                for (var j = 0, col; col = row.cells[j]; j++) {
                    ...
                    if(row.cells.length==1){					
						if (col.tagName.toLowerCase() === 'th') {						
							rowData.push({content: col.innerText, colSpan: 8, styles: {halign: 'left', fontStyle: 'bold'}});
						}
                        else{
            ...
            var titleLine3 = $("#customer option:selected").text();
            if($("#customer").val()=='' && reporttype==1){
                titleLine3 = 'All Debtors';
            }
            else if($("#customer").val()=='' && reporttype==2){
                titleLine3 = 'All Debtors Age Analysis';
            }
...
document.getElementById('btnexcelconvert').addEventListener('click', function () {
    var reporttype = $('#reporttype').val();
    var reportTitle = $("#customer option:selected").text() + ' - Statement';
    var periodTitle = $('#fromdate').val() + ' / ' + $('#todate').val();
    if($("#customer").val()=='' && reporttype==1){
        reportTitle = 'All Debtor';
    }
    else if($("#customer").val()=='' && reporttype==2){
        reportTitle = 'All Debtor Age Analysis';
        var periodTitle = $('#todate').val();
    }

    var table       = document.getElementById('debtorStatementTable');
    var fileTitle   = reportTitle.replace(/[^a-z0-9]+/gi, '_') + '_';
