...
<script>
    $(document).ready(function() {
        ...
        $("#supplier").select2({
            ...
            ajax: {
                url: "<?php echo base_url() ?>Paymentcreate/Getsupplierlist",
                type: "post",
                dataType: 'json',
                delay: 250,
        ...
        $('#dataTable').DataTable({
            ...
            "serverSide": true,
            ajax: {
                url: "<?php echo base_url() ?>scripts/paymentcreatelist.php",
            ...
            "columns": [
                ...
                },                
                {
                    ...
                    "render": function(data, type, full) {
                        var button='';
                        button+='<button class="btn btn-dark btn-sm btnview mr-1" id="'+full['idtbl_expence_info']+'"><i class="fas fa-print"></i></button>';
                        if(full['poststatus']==0){
                            if(editcheck==1){
                                button+='<button class="btn btn-primary btn-sm btnEdit mr-1" id="'+full['idtbl_expence_info']+'"><i class="fas fa-pen"></i></button>';
                            }
                            if(statuscheck==1 && full['status']==1){
                                button+='<button type="button" data-url="Paymentcreate/Paymentcreatestatus/'+full['idtbl_expence_info']+'/2" data-actiontype="2" class="btn btn-success btn-sm mr-1 btntableaction"><i class="fas fa-check"></i></button>';
                            }else if(statuscheck==1 && full['status']==2){
                                button+='<button type="button" data-url="Paymentcreate/Paymentcreatestatus/'+full['idtbl_expence_info']+'/1" data-actiontype="1" class="btn btn-warning btn-sm mr-1 btntableaction"><i class="fas fa-times"></i></button>';
                            }
                            if(deletecheck==1){
                                button+='<button type="button" data-url="Paymentcreate/Paymentcreatestatus/'+full['idtbl_expence_info']+'/3" data-actiontype="3" class="btn btn-danger btn-sm btntableaction"><i class="fas fa-trash-alt"></i></button>';
                            }
                        }
        ...
        $('#dataTable tbody').on('click', '.btnEdit', async function() {
            ...
            if (r == true) {
                ...
                $.ajax({
                    ...
                    url: '<?php echo base_url() ?>Paymentcreate/Paymentcreateedit',
                    success: function(result) { //alert(result);
                        ...
                        $('#invoice').val(obj.grnno);                       
                        $('#invoicedate').val(obj.grndate);                       
                ...
                Swal.fire({
                    ...
                    didOpen: () => {
                        ...
                        $.ajax({
                            ...
                            url: 'Paymentcreate/Paymentcreateinsertupdate',
                            success: function (result) { //alert(result);
                                // console.log(result);
                                Swal.close();
                                var obj = JSON.parse(result);
