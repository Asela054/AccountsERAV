...
<script>
    $(document).ready(function() {
        ...
        $("#supplier").select2({
            ...
            ajax: {
                url: "<?php echo base_url() ?>Payablesegregation/Getsupplierlist",
                type: "post",
                dataType: 'json',
                delay: 250,
        ...
        $('#dataTable').DataTable({
            ...
            "serverSide": true,
            ajax: {
                url: "<?php echo base_url() ?>scripts/accountpayablelist.php",
            ...
            "columns": [
                {
                    "data": "idtbl_account_payable_main"
                },
                {
                    "data": "company"
                },
                ...
                },                
                {
                    ...
                    "render": function(data, type, full) {
                        ...
                        if(full['poststatus']==0){
                            if(editcheck==1){
                                button+='<button type="button" class="btn btn-primary btn-sm btnEdit mr-1" id="'+full['idtbl_account_payable_main']+'"><i class="fas fa-pen"></i></button>';
                            }
                            if(full['status']==1 && statuscheck==1){
                                button+='<button type="button" data-url="Payablesegregation/Payablesegregationstatus/'+full['idtbl_account_payable_main']+'/2" data-actiontype="2" class="btn btn-success btn-sm mr-1 btntableaction"><i class="fas fa-check"></i></button>';
                            }else if(full['status']==2 && statuscheck==1){
                                button+='<button type="button" data-url="Payablesegregation/Payablesegregationstatus/'+full['idtbl_account_payable_main']+'/1" data-actiontype="1" class="btn btn-warning btn-sm mr-1 text-light btntableaction"><i class="fas fa-times"></i></button>';
                            }
                            if(deletecheck==1){
                                button+='<button type="button" data-url="Payablesegregation/Payablesegregationstatus/'+full['idtbl_account_payable_main']+'/3" data-actiontype="3" class="btn btn-danger btn-sm text-light btntableaction"><i class="fas fa-trash-alt"></i></button>';
                            }
                        }
        ...
        $('#dataTable tbody').on('click', '.btnEdit', async function() {
            ...
            if (r == true) {
                ...
                $.ajax({
                    ...
                    url: '<?php echo base_url() ?>Payablesegregation/Payablesegregationedit',
                    success: function(result) { //alert(result);
                        ...
                        var html;
                        html += '<option value="' + obj.supplier + '">';
                        html += obj.suppliername;
                        html += '</option>';
        ...
        $('#dataTable tbody').on('click', '.btnview', function() {
            ...
            $.ajax({
                ...
                url: '<?php echo base_url() ?>Payablesegregation/Getviewpostinfo',
                success: function(result) { //alert(result);
                    var obj = JSON.parse(result);
        ...
        $('#btnaddtolist').click(function(){
            ...
            if (!$("#segregationform")[0].checkValidity()) {
                ...
                $('#tablesegregate> tbody:last').append('<tr><td class="d-none">' + chartdetailaccountID + '</td><td>' + chartdetailaccount + '</td><td>' + narration + '</td><td class="transtype text-center">' + tratype + '</td><td class="debitamount text-right">' + debitamount + '</td><td class="creditamount text-right">' + creditamount + '</td><td class="d-none">' + accounttype + '</td></tr>');
                ...
                Swal.fire({
                    ...
                    didOpen: () => {
                        ...
                        $.ajax({
                            ...
                            url: 'Payablesegregation/Payablesegregationposting',
                            success: function (result) { //alert(result);
                                // console.log(result);
                                Swal.close();
                                var obj = JSON.parse(result);
                                if (obj.status == 1) {
                                    setTimeout( function(){ 
                                        ...
                                        $('#btnposttransaction').html('<i class="fas fa-exchange-alt mr-2"></i> Post Transaction');
                                    } ,3000 );
                                }
                                action(obj.action);
                            },
