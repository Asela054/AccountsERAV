...
class AccountPayment extends CI_Controller {
	...
	private $payment_detail_total; // store-total-of-payment-detail-to-be-passed-with-create-method
	...
	public function view(){
		$user_org_id = $this->session->userdata('companyid');
		$user_branch_id = $this->session->userdata('branchid');
		
		$result['menuaccess']=$this->Commeninfo->Getmenuprivilege();
		
		$result['user_org_id'] = $user_org_id;
		$result['user_branch_id'] = $user_branch_id;
		...
		$account_period=get_account_period($user_org_id, $user_branch_id);//'';
		...
		$result['account_period_id']=$account_period_master_id;
		$result['account_period_txt']=$account_period_txt;
		
		$result['child_accounts']=get_child_account_list($user_org_id, $user_branch_id, false);//rs_to_kv(new stdClass());//
	...
	public function store(){
		...
		if($this->input->post('child_account_id')!='-1'){
			$this->form_validation->set_rules('acc_detail_amount', 'Accountable value', 'required');
		}
		...
		if($this->form_validation->run() == false) {
			...
			if($validationErrors == ''){
				$validationErrors = form_error('acc_detail_amount', '<div class="errormsg">', '</div>');
			}
			...
			$headerData = array('tradate'=>$suppInvoiceDate, 'supplier'=>$supplierId, 'paytype'=>1, 
			...
			$res_info = $this->AccountPaymentinfo->regUpdateOrder($headerData, $accPayableMainId, 
																  $revisedStatusData,
																  $detailData, $detailData_idtbl_account_payable
																  );
	...
	*/
	...
	public function payment_check($field_val){
		$result=$this->AccountPaymentinfo->verifyPaymentTotal($field_val);
	...
	public function doc_check($field_val){
		...
		$result=$this->AccountPaymentinfo->getOrderHeader($order_id, '');
		...
		if($head_status){
			$this->form_validation->set_message('doc_check', 'Changes of invoice total amount has to be updated');
			return false;
		}
	...
	public function approve(){
		...
		$this->form_validation->set_rules('supp_invoice_amount', 'Invoice total amount', 'callback_doc_check');
		
		$this->form_validation->set_rules('selected_opt', 
										  'Invoice payments', 
										  'callback_post_check|callback_payment_check' // callback_approve_check
										  );
		...
		if($this->form_validation->run() == false) {
			...
			$res_info = $this->Payablesegregationinfo->Payablesegregationposting();//regApproveOrder($payableMainId);
			
			die();
			
			$data = array();
			$data['resMsg'] = $res_info['message'];//'Successfully approved';
			$data['resTheme'] = $res_info['icon'];//'bg-info text-white';
		}
