...
**/
	...
	function getSpecialCateDetailAccount($searchTerm){
		$CI = get_instance();
		$sql="SELECT `idtbl_material_main_category` AS `typeid`, `main_category` AS `type`, '1' AS `typecate` FROM `tbl_material_main_category` WHERE `status`=?";
		if(!empty($searchTerm)){
			$sql.=" AND `main_category` LIKE '%$searchTerm%'";
		}
		$sql.=" UNION ALL ";
		$sql.="SELECT `idtbl_material_category` AS `typeid`, `categoryname` AS `type`, '2' AS `typecate` FROM `tbl_material_category` WHERE `status`=?";
		if(!empty($searchTerm)){
			$sql.=" AND `categoryname` LIKE '%$searchTerm%'";
		}
