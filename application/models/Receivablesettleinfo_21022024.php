...
class Receivablesettleinfo extends CI_Model{
    ...
    public function Getinvoiceaccocustomer(){
        ...
        $this->db->select('`tbl_sales_info`.`idtbl_sales_info`, `tbl_sales_info`.`invno`, `tbl_sales_info`.`amount`, IFNULL(SUM(`tbl_receivable_info`.`amount`), 0) AS `sumpay`, (`tbl_sales_info`.`amount`-IFNULL(SUM(`tbl_receivable_info`.`amount`), 0)) AS `balpay`');
    ...
    public function Receivablesettleinsertupdate(){
        ...
        $invoicedata=$this->input->post('tableData');

        if($receivabletype==1){$detailaccount=$chartofdetailaccount;}
        else if($receivabletype==2){$chartaccount=$chartofdetailaccount;}
        ...
        if($recordOption==1){
            if(!empty($batchno)){
                ...
                $receivalbleID=$this->db->insert_id();
                ...
                foreach($invoicedata as $rowinvoicedata){
                    ...
                    $datasub = array(
                        ...
                        'insertdatetime'=> $updatedatetime, 
                        'tbl_user_idtbl_user'=> $userID,
                        'tbl_receivable_idtbl_receivable'=> $receivalbleID,
                    );
    ...
    public function Receivablesettleposting(){
        ...
        $this->db->select('recdate, batchno, amount, poststatus, status, editstatus, postviewtime, updatedatetime, tbl_company_idtbl_company, tbl_company_branch_idtbl_company_branch, tbl_master_idtbl_master, payer, tbl_account_idtbl_account, narration');
        $this->db->from('tbl_receivable');
        $this->db->where('idtbl_receivable', $recordID);
        $this->db->where('status', 1);
