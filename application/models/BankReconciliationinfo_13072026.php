...
class BankReconciliationinfo extends CI_Model{
	...
	*/
	...
	public function getOrderDetail($accRef, $bankRecId, $bankRecBatchNo, $maxYear, $maxMonth, $matchConfirm, $chkConfirm){
		...
		/* Load transactions for the selected financial year+month AND any earlier
		   period whose transactions are still unmatched (ismatch=0 via outer WHERE) —
		   so uncleared items from prior months carry forward as outstanding until cleared. */
		$sql .= "INNER JOIN ( ";
	...
	public function getBankDepositDetails($bankRecId, $maxYear, $maxMonth, $depositConfirm){
		...
		*/
		// Carry forward: selected period AND any earlier period (still unmatched via depositstatus=? in outer WHERE)
		...
		$qry_deposit = $this->db->query($sql, array($maxYear, $maxMonth, $maxYear, $maxYear, $maxMonth, $bankRecId, $depositConfirm));
