...
class Payreceivepostinfo extends CI_Model{
    public function Getpayreceivelist(){
        ...
        if($filtortype==1){
            ...
            $this->db->where('tbl_account_payable_main.poststatus', '0');
            ...
            foreach($respond->result() as $rowrespond){
                ...
                $this->db->select('tbl_account_payable.idtbl_account_payable as `detailid`,tbl_account_payable.narration, tbl_account_payable.amount, tbl_account_detail.accountno, tbl_account_detail.accountname, tbl_account.accountno AS chartaccountno, tbl_account.accountname AS chartaccountname');
            ...
            // $this->db->where('tbl_account_receivable_main.poststatus', '0');
            ...
            foreach($respond->result() as $rowrespond){
                ...
                $this->db->select('tbl_account_receivable.idtbl_account_receivable as `detailid`, tbl_account_receivable.narration, tbl_account_receivable.amount, tbl_account_detail.accountno, tbl_account_detail.accountname, tbl_account.accountno AS chartaccountno, tbl_account.accountname AS chartaccountname');
            ...
            $this->db->select('tbl_pettycash.idtbl_pettycash, tbl_pettycash.date, tbl_pettycash.pettycashcode, tbl_pettycash.amount, tbl_pettycash.desc, tbl_pettycash.poststatus, tbl_company.company, tbl_company_branch.branch, tbl_account_detail.accountno, tbl_account_detail.accountname, tbl_pettycash.tbl_company_idtbl_company, tbl_account.accountno AS chartaccountno, tbl_account.accountname AS chartaccountname');
            ...
            $this->db->where('tbl_pettycash.poststatus', '0');
            ...
            foreach($respond->result() as $rowrespond){
                ...
                $obj->chartaccountno=$rowrespond->chartaccountno;
                $obj->chartaccountname=$rowrespond->chartaccountname;
                $obj->desc=$rowrespond->desc;

                array_push($listarray, $obj);
            }
        }
        else if($filtortype==4){
            $this->db->select('tbl_receivable.idtbl_receivable, tbl_receivable.recdate, tbl_receivable.batchno, tbl_receivable.amount, tbl_receivable.narration, tbl_receivable.poststatus, tbl_company.company, tbl_company_branch.branch, tbl_account_detail.accountno, tbl_account_detail.accountname, tbl_receivable.tbl_company_idtbl_company, tbl_account.accountno AS `chartaccount`, tbl_account.accountname AS `chartaccountname`');
            ...
            $this->db->where('MONTH(tbl_receivable.recdate)', $month);
            $this->db->where('YEAR(tbl_receivable.recdate)', $year);
            $this->db->where('tbl_receivable.poststatus', '0');
            $this->db->where('tbl_receivable.tbl_receivable_type_idtbl_receivable_type', '2');
            $this->db->where('tbl_receivable.status', '1');
            $this->db->where('tbl_receivable.tbl_company_idtbl_company', $company);
            $this->db->where('tbl_receivable.tbl_company_branch_idtbl_company_branch', $branch);
            ...
            foreach($respond->result() as $rowrespond){
                ...
                $this->db->where('idtbl_receivable', $rowrespond->idtbl_receivable);
                $this->db->where('poststatus', 0);
                $this->db->update('tbl_receivable', $data);

                $obj=new stdClass();
                $obj->id=$rowrespond->idtbl_receivable;
                $obj->tradate=$rowrespond->recdate;
                $obj->batchno=$rowrespond->batchno;
                $obj->amount=$rowrespond->amount;
                $obj->poststatus=$rowrespond->poststatus;
                $obj->company=$rowrespond->company;
                ...
                $obj->accountno=$rowrespond->accountno;
                $obj->accountname=$rowrespond->accountname;
                $obj->chartaccountno=$rowrespond->chartaccount;
                $obj->chartaccountname=$rowrespond->chartaccountname;
                $obj->desc=$rowrespond->narration;
        ...
        if($filtortype>2){
            foreach($listarray AS $rowlist){
                ...
                <tr class="'; if($rowlist->poststatus==1){$html.='table-success';} $html.='">
                    ...
                    $html.='</td>
                    <td colspan="2">';
                    if(!empty($rowlist->accountno)){$html.=$rowlist->accountno;}
                    else{$html.=$rowlist->chartaccountno;}
                    $html.='</td>
            ...
            foreach($listarray AS $rowlist){
                ...
                foreach($rowlist->detailinfo AS $rowdetaillist){
                    ...
                    <tr class="'; if($rowlist->poststatus==1){$html.='table-success';} $html.='">
                        ...
                        $html.='</td>
                        <td colspan="2">';
                        if(!empty($rowdetaillist->accountno)){$html.=$rowdetaillist->accountno;}
                        else{$html.=$rowdetaillist->chartaccountno;}
                        $html.='</td>
    ...
    public function Payreceivepostposting(){
        ...
        if($filtortype==1){
            foreach($datalist as $rowdatalist){
                ...
                if($respond->row(0)->poststatus==0 && $respond->row(0)->status==1 && $respond->row(0)->editstatus==0){
                    if($respond->row(0)->postviewtime>$respond->row(0)->updatedatetime){
                        ...
                        $this->db->select('`idtbl_account_payable`, `tradate`, `batchno`, `amount`, `narration`, `tbl_master_idtbl_master`, `tbl_company_idtbl_company`, `tbl_company_branch_idtbl_company_branch`, `tbl_account_detail_idtbl_account_detail`, `tbl_account_idtbl_account`');
                        $this->db->from('tbl_account_payable');
            ...
            foreach($datalist as $rowdatalist){
                ...
                if($respond->row(0)->poststatus==0 && $respond->row(0)->status==1 && $respond->row(0)->editstatus==0){
                    if($respond->row(0)->postviewtime>$respond->row(0)->updatedatetime){
                        ...
                        //Other account Transaction
                        $this->db->select('`idtbl_account_receivable`, `tradate`, `batchno`, `amount`, `narration`, `tbl_master_idtbl_master`, `tbl_company_idtbl_company`, `tbl_company_branch_idtbl_company_branch`, `tbl_account_detail_idtbl_account_detail`, `tbl_account_idtbl_account`');
                        $this->db->from('tbl_account_receivable');
            ...
            foreach($datalist as $rowdatalist){
                ...
                $this->db->select('recdate, batchno, amount, poststatus, status, editstatus, postviewtime, updatedatetime, tbl_company_idtbl_company, tbl_company_branch_idtbl_company_branch, tbl_master_idtbl_master, payer, tbl_account_idtbl_account, tbl_account_detail_idtbl_account_detail, narration');
                $this->db->from('tbl_receivable');
                ...
                if($respond->row(0)->poststatus==0 && $respond->row(0)->status==1 && $respond->row(0)->editstatus==0){
                    if($respond->row(0)->postviewtime>$respond->row(0)->updatedatetime){
                        ...
                        if(!empty($respond->row(0)->tbl_account_detail_idtbl_account_detail)){
                            $chartofaccountinfo=get_chart_account_acco_child_account($company, $branch, $respond->row(0)->tbl_account_detail_idtbl_account_detail);
                            $chartofaccountID=$chartofaccountinfo->row(0)->idtbl_account;
                        }
                        else{
