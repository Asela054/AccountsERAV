...
class BankReconciliationinfo extends CI_Model{
	...
	*/
	...
	public function getAccountHeader($bankAcc, $selectedYear = null, $selectedMonth = null){
		$sql_acc = "SELECT *, count(*) AS ucnt, acc_rec_batchno as rec_batchno 
					FROM tbl_bank_rec_list 
					WHERE rec_approved = 0 
					AND tbl_account_idtbl_account = ?";
		$result_acc = $this->db->query($sql_acc, array($bankAcc));
		$row_acc    = $result_acc->row();
		...
		// ── FIX: selected year/month ලෙස last approved rec find කරනවා ──
		// User April select කළාම → April ට BEFORE approved rec find කරනවා
		// User August select කළාම → August ට BEFORE approved rec find කරනවා
	
		if(!empty($selectedYear) && !empty($selectedMonth)){
	
			// Selected period ට BEFORE ලේ last approved rec
			...
			// Fallback: original behaviour (latest approved rec)
			$sql_approved = "SELECT MAX(idtbl_bank_rec_list) AS last_approved_rec
							FROM tbl_bank_rec_list
							WHERE rec_approved = 1
							AND tbl_account_idtbl_account = ?";
	...
	public function getOrderDetail($accRef, $bankRecId, $bankRecBatchNo, $maxYear, $maxMonth, $matchConfirm, $chkConfirm){
		...
		/* ── FIX: Year+Month exact period filter ──────────────────────────────
		
		පරණ code (bug):
			tbl_finacial_year_idtbl_finacial_year  <= ?   (year)
			tbl_finacial_month_idtbl_finacial_month <= ?   (month)

		ගැටලුව:
			Year 2025/2026, Month = April (id=4) select කළාම
			Month id <= 4 condition නිසා January(1), February(2),
			March(3), April(4) — ඒ සියල්ල load වෙනවා.
			
			ඊටත් වඩා නරකයි: Year <= 2025/2026 AND Month <= 4 කියන්නේ
			නිදසුනක් ලෙස Year=2024/2025, Month=8 (August) record ද
			(year condition pass, month 8 > 4 නිසා filter) — 
			නමුත් cross-year scenarios ලේ incorrectly include/exclude වෙනවා.

		Fix:
			Exact selected year+month ලේ transactions ONLY load කරනවා.
			Bank reconciliation = specific period එකක් reconcile කිරීම,
			ඒ නිසා exact match කරන්නට ඕනෑ.

			(year < maxYear)  → previous years ALL months ok
			OR
			(year = maxYear AND month <= maxMonth) → same year, up to selected month
		─────────────────────────────────────────────────────────────────────── */
		$sql .= "INNER JOIN ( ";
		...
		$sql .= "  WHERE status = 1 ";
		$sql .= "    AND ( ";
		$sql .= "          tbl_finacial_year_idtbl_finacial_year < ? ";           /* $maxYear  - previous years */
		$sql .= "          OR ( ";
		$sql .= "               tbl_finacial_year_idtbl_finacial_year = ? ";      /* $maxYear  - same year */
		$sql .= "               AND tbl_finacial_month_idtbl_finacial_month <= ? "; /* $maxMonth - up to selected month */
		$sql .= "             ) ";
		$sql .= "        ) ";
		$sql .= ") AS drv_master ";
		...
		/*
		* Parameter order (9 values):
		*  1. $chkConfirm    → COALESCE 1st fallback
		*  2. $maxYear       → COALESCE year diff
		*  3. $maxMonth      → COALESCE month diff
		*  4. $matchConfirm  → ismatch filter
		*  5. $accRef        → tbl_account_idtbl_account
		*  6. $bankRecBatchNo→ batchno exclude
		*  7. $maxYear       → drv_master year <   (previous years)
		*  8. $maxYear       → drv_master year =   (same year)
		*  9. $maxMonth      → drv_master month <= (up to selected month)
		* 10. $bankRecId     → tbl_bank_rec_list FK
		*/
		$qry_transactions = $this->db->query($sql, array(
			$chkConfirm,
			$maxYear, $maxMonth,
			$matchConfirm, $accRef, $bankRecBatchNo,
			$maxYear,           /* year < (previous years)  */
			$maxYear, $maxMonth,/* year = AND month <=       */
			$bankRecId
		));
