...
class AccountPaymentinfo extends CI_Model{
	...
	public function verifyPaymentTotal($orderRef){
		$sql = "SELECT count(*) as c_match FROM `tbl_account_payable_main` cross join (select SUM(amount) as d_amount from tbl_account_payable where tbl_account_payable_main_idtbl_account_payable_main=? AND status=1) as drv_inv ";
		$sql .= "WHERE `tbl_account_payable_main`.`idtbl_account_payable_main`=? AND `tbl_account_payable_main`.`amount`=drv_inv.d_amount";
	...
	public function getOrderDetail($orderRef){
		...
		$this->db->where_in('tbl_account_payable.status', array(1,2));
	...
	public function regUpdateOrder($headerData, $headerDataExistingId, $revisedData, $detailData, $detailDataExistingId){
		...
		*/
		...
		if(!empty($revisedData)){
			...
			//$this->db->set('updatedatetime', 'NOW()', FALSE);
			$this->db->update_batch('tbl_account_payable', $revisedData, 'idtbl_account_payable'); 
		}
		...
		if(($head_k>0)&&(!empty($detailData))){//echo 3;
			if(empty($detailDataExistingId)){//echo '3.1';
				if($detailData['tbl_account_detail_idtbl_account_detail']!='-1'){
					// set qrytime field
					$this->db->set('insertdatetime', 'NOW()', FALSE);
