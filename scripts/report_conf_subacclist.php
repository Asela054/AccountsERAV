...
// DB table to use
$table = 'tbl_account';//'tbl_subaccount';

// Table's primary key
$primaryKey = 'idtbl_account';//'idtbl_subaccount';
...
$joinQuery = "FROM (SELECT idtbl_account as `idtbl_subaccount`, accountno as `subaccount`, CONCAT(`accountno`, ' ', `accountname`) AS subaccountname FROM `tbl_account` WHERE `status`=1) AS `u` LEFT OUTER JOIN (SELECT `id` AS conf_id, `tbl_account_idtbl_account`, `report_part_cancel` FROM `tbl_gl_report_sub_section_particulars` WHERE `tbl_gl_report_sub_section_id`=".$optSection.") AS drv ON `u`.`idtbl_subaccount`=`drv`.`tbl_account_idtbl_account`";
/*
