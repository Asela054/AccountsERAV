...
// DB table to use
$table = 'tbl_account_paysettle';

// Table's primary key
$primaryKey = 'idtbl_account_paysettle';
...
// indexes
$columns = array(
    array( 'db' => '`u`.`idtbl_account_paysettle`', 'dt' => 'idtbl_account_paysettle', 'field' => 'idtbl_account_paysettle' ),
    ...
    array( 'db' => '`u`.`chedate`',                 'dt' => 'chedate',                 'field' => 'chedate' ),
    array( 'db' => '`u`.`paymentno`',               'dt' => 'paymentno',               'field' => 'paymentno' ),
    array( 'db' => '`u`.`status`',                  'dt' => 'status',                  'field' => 'status' ),
    array( 'db' => '`u`.`paymentfiltertype`',       'dt' => 'paymentfiltertype',       'field' => 'paymentfiltertype' ),
);
...
$joinQuery = "FROM (
    ...
    FROM `tbl_account_paysettle` AS `u` 
    ...
    LEFT JOIN `$tablename` AS `ui` 
        ON (`ui`.`$column1` = `u`.`supplier`) 
    LEFT JOIN `tbl_account_paysettle_has_tbl_cheque_issue` AS `uj` 
        ON (`uj`.`tbl_account_paysettle_idtbl_account_paysettle` = `u`.`idtbl_account_paysettle`) 
    LEFT JOIN `tbl_cheque_issue` AS `uk` 
        ON (`uk`.`idtbl_cheque_issue` = `uj`.`tbl_cheque_issue_idtbl_cheque_issue`) 
    LEFT JOIN `tbl_cheque_info` AS `ul` 
        ON (`ul`.`idtbl_cheque_info` = `uk`.`tbl_cheque_info_idtbl_cheque_info`) 
    LEFT JOIN `tbl_bank` AS `um` 
        ON (`um`.`idtbl_bank` = `ul`.`tbl_bank_idtbl_bank`)
) AS `u`";
