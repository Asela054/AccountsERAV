...
$joinQuery = "FROM (select *, concat(tbl_company_idtbl_company, '_', tbl_company_branch_idtbl_company_branch) as sect_key from `tbl_account_payable_main` where paytype=1 and status IN (1,2)) as u inner join (select idtbl_supplier, suppliername from tbl_supplier where status=1) as drv_supplier on u.supplier=drv_supplier.idtbl_supplier";
